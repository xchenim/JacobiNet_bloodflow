...
def shear_centerlines(ctrl_points, shear_strength=0.15):
    ...
    if X_shear:
        shear_matrix = np.array(
            [[1, 0, 0, 0], [Sy, 1, 0, 0], [Sz, 0, 1, 0], [0, 0, 0, 1]]
        )
    else:
        shear_matrix = np.array(
            [[1, Sx, 0, 0], [0, 1, 0, 0], [0, Sz, 1, 0], [0, 0, 0, 1]]
        )

    homogeneous_curve_points = np.concatenate(
        (ctrl_points, np.ones((len(ctrl_points), 1))), axis=1
    )
...
def warp1(data_original, ws):
    ...
    x += dx * (
        c1 * np.sin((np.pi) * x / lx)
        + c1 * np.cos(np.pi * x / lx)
        + c2 * np.sin(2 * np.pi * x / lx)
        + c3 * np.sin(3 * np.pi * x / lx)
        + c4 * np.sin(5 * np.pi * x / lx)
    )
    y += dy * (
        c1 * np.sin((np.pi) * y / ly)
        + c1 * np.cos(np.pi * y / ly)
        + c2 * np.sin(2 * np.pi * y / ly)
        + c3 * np.sin(3 * np.pi * y / ly)
        + c4 * np.sin(5 * np.pi * y / ly)
    )
    z += dz * (
        c1 * np.sin((np.pi) * z / lz)
        + c1 * np.cos(np.pi * z / lz)
        + c2 * np.sin(2 * np.pi * z / lz)
        + c3 * np.sin(3 * np.pi * z / lz)
        + c4 * np.sin(5 * np.pi * z / lz)
    )
